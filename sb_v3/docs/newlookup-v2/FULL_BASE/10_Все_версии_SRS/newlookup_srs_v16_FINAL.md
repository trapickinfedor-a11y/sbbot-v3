# Newlookup — Техническое Задание v16.0 (Максимальная Детализация)
...
> **Что новое в v16:** лимиты ботов, система доступа по депозитам (Basic/Advanced/Pro), Marketer Bot без ссылок — только создание бота, расширенная CRM (360° карточка пользователя), Seller Bot уведомления, Seller Mini App полная структура, закреплённые сообщения, рассылки с закреплением, 4 языка.
...
### 3.1. Полная таблица ролей и прав

| Роль | Код | Интерфейс | Права |
| :--- | :--- | :--- | :--- |
| **Покупатель** | `buyer` | Mirror Bot | Просмотр каталога, покупка, споры, рейтинг. |
| **Продавец** | `seller` | Seller Bot + Mini App | Загрузка товаров, управление командой, вывод средств. |
| **Маркетолог** | `marketer` | Marketer Bot | Управление ботами-прокси, просмотр своей статистики. |
| **Воркер** | `worker` | Worker Bot + CRM | Выполнение заказов Per Order, вывод средств. |
| **Поддержка** | `support` | Support Bot | Ответы на тикеты покупателей. |
| **Модератор** | `moderator` | Support Bot | Разрешение споров, модерация товаров. |
| **Финансист** | `finance` | Support Bot | Одобрение/отклонение выводов средств. |
| **Администратор** | `admin` | Support Bot + Admin Panel | Управление каталогом, ценами, купонами, рассылками. |
| **Аудитор** | `auditor` | Admin Panel (read-only) | Просмотр аудит-логов и финансовой отчётности. |
| **Владелец** | `owner` | Main Bot + Admin Panel | Полный доступ ко всему. |

### 3.2. Матрица доступа к Admin Panel
...
- Поле: Password
- Кнопка: [Login]
- 2FA: После ввода логина/пароля — запрос кода из Telegram (или TOTP).
...
| Карточка | Данные |
| :--- | :--- |
| MRR | $12,450.00 (+8% vs last month) |
| New Users Today | 47 |
| Active Users (7d) | 312 |
| Avg Order Value | $85.40 |
| Pending Withdrawals | 4 ($2,100.00) |
| Open Disputes | 2 |
| Products Pending Review | 7 |
...
### 12.3. Управление каталогом

**Страница "Menu Categories":**
...
| Название ES | Название на испанском. |
| Название ZH | Название на китайском. |
| Порядок | Числовое значение (drag-and-drop). |
| Статус | Active / Hidden. |
| Действия | Редактировать / Скрыть / Удалить. |

**Кнопка:** [➕ Add Category]

**Форма добавления/редактирования категории:**
- Иконка (emoji picker).
- Название на 4 языках.
- Тип: `in_stocks` / `per_order` / `preorder`.
- Статус: Active / Hidden.
- Привязка воркеров (для Per Order): мультиселект.

### 12.4. Управление ценами

**Страница "Service Prices":**

Таблица с колонками: Услуга / Текущая цена / Последнее изменение / Изменить.

| Услуга | Цена |
| :--- | :--- |
| Seller registration fee | $600.00 |
| Platform commission (default) | 15% |
| Credit Score | $35.00 |
| Full Credit Report | $65.00 |
| Phone lookup | $5.00 |
| Address lookup | $8.00 |
| Email reputation | $3.00 |
| People Search | $25.00 |
| Worker share (Per Order) | 80% |

### 12.5. Управление купонами

**Страница "Coupons":**

Таблица купонов с колонками: Код / Тип / Размер / Использований / Лимит / Срок / Статус / Действия.

**Форма создания купона:**

| Поле | Тип | Описание |
| :--- | :--- | :--- |
| Код | Text | Уникальный код. Кнопка [🎲 Generate Random]. |
| Тип скидки | Select | Фиксированная сумма / Процент. |
| Размер скидки | Number | Значение. |
| Минимальная сумма заказа | Number | 0 = без ограничений. |
| Лимит использований | Number | 0 = безлимит. |
| Один раз на пользователя | Checkbox | Да/Нет. |
| Область применения | Select | Всё / Категория / Товар. |
| Категория/Товар | Select (если выбрано) | Выбор из списка. |
| Дата начала | DatePicker | Начало действия. |
| Дата окончания | DatePicker | Конец действия (необязательно). |
| Активен | Toggle | Да/Нет. |
...
| Поле | Тип | Описание |
| :--- | :--- | :--- |
| Текст (EN) | Textarea | Текст на английском. |
| Текст (RU) | Textarea | Текст на русском. |
| Текст (ES) | Textarea | Текст на испанском. |
| Текст (ZH) | Textarea | Текст на китайском. |
| Медиа | File Upload | Фото или видео (необязательно). |
| Кнопка | Text + URL | Текст кнопки и ссылка (необязательно). |
| Аудитория | Select | Все / По языку / По уровню лояльности. |
| Время отправки | DateTimePicker | Сейчас или отложенная. |

**Предпросмотр:** Кнопка [👁️ Preview] показывает как будет выглядеть сообщение.

### 12.7. Модерация

**Вкладка "Products":**

Таблица товаров на модерации: Продавец / Категория / Цена / Дата подачи / Действия (Одобрить / Отклонить / Просмотреть).

**Вкладка "Sellers":**

Таблица заявок новых продавцов: @username / Дата регистрации / Депозит оплачен / Действия.

**Вкладка "Disputes":**

Таблица споров: ID спора / Заказ / Покупатель / Продавец / Сумма / Статус / Дата / Действия.

### 12.8. Управление Brute Bank

**Страница "Brute Bank Groups":**

Таблица групп: Банк / Атрибуты / Цена / Количество / Продано / Действия.

**Конфигуратор атрибутов (визуальный):**
...
Integrations:
[✅ YODLEE] [☐ FINICITY] [✅ PLAID] [☐ MX] [☐ AKOYA]
[➕ Add Custom Integration: ____________]

Generated string: AN:RN+INST YODLEE+INST PLAID
...
**Страница "eSIM Tariffs":**

Таблица тарифов: Название / Страна / Трафик / Срок / Цена / Активен / Действия.

**Форма добавления тарифа:**
- Название (например, "USA 10GB 30 days").
- Страна (select с флагами).
- Объём трафика (GB).
- Срок действия (дни).
- Тип: Data only / Data + Calls + SMS.
- Цена (USD).
- Активен (toggle).

**Обработка заказов eSIM:**

Страница "eSIM Orders" — таблица заказов с кнопкой [📤 Upload QR/File] для каждого ожидающего заказа.

### 12.10. Предзаказы (Accounts)

**Страница "Pre-orders":**

Таблица: ID / Покупатель / Тип аккаунта / Статус / Дата / Действия.

**Обработка предзаказа:**
- Кнопка [📤 Upload Account Data] — загрузить файл с данными аккаунта.
- После загрузки — данные автоматически отправляются покупателю в Mirror Bot.

### 12.11. Аналитика

**Разделы:**
- **Воронка:** Регистрации → Пополнения → Заказы → Повторные заказы.
- **Когорты:** Таблица удержания по неделям.
- **Каналы привлечения:** Откуда пришли пользователи (прямой / реферал / маркетолог / UTM).
- **Сравнение периодов:** Выбор двух периодов для сравнения.
- **Экспорт:** [📥 Export CSV] / [📥 Export Excel].

### 12.12. Управление автоматизациями

### 12.13. CRM (Customer Relationship Management)

**Концепция:** CRM — это центральный узел для управления всеми пользователями системы. Он должен предоставлять 360-градусный обзор каждого пользователя, его действий, финансов и истории взаимодействия.

**12.13.1. Профиль пользователя**

При поиске и выборе пользователя открывается его детальный профиль с несколькими вкладками:

| Вкладка | Содержание |
| :--- | :--- |
| **Обзор** | **Ключевая информация:** ID, @username, роли, баланс, дата регистрации, последний визит, язык, статус (Active/Banned/Muted). **Действия:** [Ban User], [Send Message], [Change Balance], [Add Note]. |
| **История заказов** | Таблица всех заказов пользователя с ID, датой, товаром, суммой, статусом. |
| **Финансовая история** | Таблица всех транзакций (пополнения, списания, выводы) с `trace_id`, датой, суммой, методом. |
| **Лог активности** | Запись всех ключевых действий пользователя в системе: логины, изменения профиля, создание тикетов и т.д. |
| **Тикеты поддержки** | Список всех тикетов, созданных пользователем, с возможностью просмотра диалога. |
| **Заметки админа** | История заметок, оставленных администраторами по данному пользователю. |

**12.13.2. Расширенный поиск и фильтрация**

- **Поисковая строка:** Поиск по ID, @username, email, IP-адресу.
- **Фильтры:**
    - По роли (buyer, seller, marketer, etc.).
    - По дате регистрации (диапазон).
    - По балансу (больше/меньше X).
    - По статусу (Active, Banned, Muted).
    - По наличию заказов / пополнений.

**12.13.3. Центр коммуникаций**

- **Прямое сообщение:** Возможность отправить личное сообщение пользователю прямо из его профиля в CRM (сообщение придёт в соответствующего бота).
- **Массовые рассылки:** Расширение существующего раздела рассылок с возможностью таргетинга на основе фильтров из CRM (например, отправить сообщение всем продавцам, зарегистрированным в прошлом месяце).

**12.13.4. Интеграция с Support Bot**

- **Просмотр тикетов:** В CRM должна быть отдельная страница для просмотра и управления всеми тикетами поддержки, аналогично интерфейсу в Support Bot.
- **Ответ на тикет:** Администратор может ответить на тикет прямо из веб-интерфейса CRM, ответ будет отправлен пользователю в Mirror Bot.

**12.13.5. Дашборд антифрода**

- **Список подозрительных:** Отдельная страница со списком пользователей, помеченных антифрод-системой.
- **Причина флага:** Для каждого пользователя указывается причина, по которой он был помечен (например, "Слишком много пополнений сразу", "Аффилированные аккаунты").
- **Действия:** [Проверен, всё в порядке], [Заморозить аккаунт], [Забанить].

**Страница "Automations":**

**Переключатели по типам:**

| Тип | Статус | Действие |
| :--- | :--- | :--- |
| Credit Score | ✅ Enabled | Toggle |
| Phone Lookup | ✅ Enabled | Toggle |
| Address Lookup | ✅ Enabled | Toggle |
| Email Reputation | ✅ Enabled | Toggle |
...
---
## 14. Спецификации товаров — Полные конфигураторы {#14}

### 14.1. Selfreg BA — Полная карточка товара

**Все поля:**

| Поле | Тип | Обязательное | Описание |
| :--- | :--- | :--- | :--- |
| `bank_name` | String | ✅ | Название банка (Chase, BoA, Wells Fargo и т.д.). |
| `account_type` | Enum + Custom | ✅ | Checking / Savings / Money Market / CD / [Свой вариант → модерация]. |
| `balance` | Decimal | ✅ | Баланс в USD. Должен быть > 0. |
| `online_access` | Boolean | ✅ | Есть ли онлайн-доступ к аккаунту. |
| `has_phone` | Boolean | ✅ | Привязан ли номер телефона. |
| `phone_renewable` | Boolean | Если `has_phone=true` | Можно ли продлить номер. |
| `phone_change_allowed` | Boolean | Если `has_phone=false` | Можно ли сменить номер. |
| `has_email` | Boolean | ✅ | Привязана ли почта. |
| `email_change_allowed` | Boolean | Если `has_email=false` | Можно ли сменить почту. |
| `email_change_in_settings` | Boolean | ✅ | Можно ли сменить почту в настройках аккаунта. |
| `state` | String (2 chars) | ✅ | Штат (CA, NY, TX, FL и т.д.). |
| `registration_date` | Date | ✅ | Дата регистрации аккаунта (вводится продавцом вручную). |
| `auto_delete_days` | Integer | ✅ | Через сколько дней авто-удалить/напомнить снизить цену. 0 = выключено. Задаёт продавец. |
| `price` | Decimal | ✅ | Цена в USD. |
| `file` | File | ❌ | Прикреплённый файл с данными (txt/pdf/zip). |

**Автоматически генерируемая карточка для покупателя (пример):**
```
🏦 Chase Bank
💳 Checking | Balance: $2,450.00
🌐 Online access: ✅
...
### 14.2. Selfreg CC — Полная карточка товара
...
| `card_name` | String | ✅ | Название карты (Chase Sapphire, Citi Double Cash и т.д.). |
...
| `phone_renewable` | Boolean | Если `has_phone=true` | Можно ли продлить. |
| `phone_change_allowed` | Boolean | Если `has_phone=false` | Можно ли сменить. |
| `has_email` | Boolean | ✅ | Привязана ли почта. |
| `email_change_allowed` | Boolean | Если `has_email=false` | Можно ли сменить. |
| `email_change_in_settings` | Boolean | ✅ | Смена в настройках. |
| `auto_delete_days` | Integer | ✅ | Авто-удаление. |
| `price` | Decimal | ✅ | Цена. |
...
🔐 VCC: ✅ | VCC Limit: $2,000.00
📍 State: CA | ZIP: 90210
📮 Billing: 123 Main St, Los Angeles, CA
...
### 14.3. Logs BA — Полная карточка товара
...
| `total_balance` | Decimal | ✅ | Суммарный баланс (авто или ручной). |
| `email_type` | Enum | ✅ | gmail / aol / yahoo / outlook / live / spectrum / other. |
| `email_valid` | Boolean | ✅ | Почта валидна. |
| `has_cvv` | Boolean | ✅ | Есть CVV. |
| `has_balance_transfer` | Boolean | ✅ | Есть Balance Transfer. |
| `has_promo` | Boolean | ✅ | Есть промо-предложения. |
| `zelle_enrolled` | Boolean | ✅ | Зарегистрирован в Zelle. |
| `wire_transfer` | Boolean | ✅ | Доступен Wire Transfer. |
| `safepass_unlocked` | Boolean | ✅ | SafePass разблокирован. |
| `has_investments` | Boolean | ✅ | Есть инвестиционный счёт. |
| `has_credit_line` | Boolean | ✅ | Есть кредитная линия. |
| `has_cookies` | Boolean | ✅ | Есть cookies. |
| `has_screenshot` | Boolean | ✅ | Есть скриншот. |
| `price` | Decimal | ✅ | Цена. |
| `file` | File | ❌ | Файл с логами. |
...
### 14.4. Brute Bank — Полная спецификация

**Атрибуты строки:**

| Код | Расшифровка | Описание |
| :--- | :--- | :--- |
| `AN` | Account Number | Номер счёта. |
| `RN` | Routing Number | Маршрутный номер. |
| `NAME` | Full Name | Полное имя владельца. |
| `ADDRESS` | Street Address | Адрес. |
| `CITY` | City | Город. |
| `ZIP` | ZIP Code | Почтовый индекс. |
| `ONLINE ACCESS` | Online Banking | Доступ к онлайн-банкингу. |

**Интеграции:**

| Код | Сервис | Описание |
| :--- | :--- | :--- |
| `YODLEE` | Yodlee | Финансовая агрегация. |
| `FINICITY` | Finicity | Верификация счётов. |
| `PLAID` | Plaid | Банковские API. |
| `MX` | MX Technologies | Финансовые данные. |
| `AKOYA` | Akoya | Open Banking. |
| Custom | Любой | Продавец вводит вручную. |

**Формат строки (генерируется автоматически):**
```
AN:RN+INST YODLEE+INST PLAID
```

**Формат файла загрузки (.txt):**
```
account_number|routing_number|full_name|address|city|zip
123456789|021000021|John Smith|123 Main St|New York|10001
987654321|026009593|Jane Doe|456 Oak Ave|Los Angeles|90001
```

**Логика массовой загрузки:**
1. Продавец выбирает группу банка (с уже настроенными атрибутами и ценой).
2. Загружает `.txt` файл.
3. Система парсит файл, проверяет количество полей в каждой строке.
4. Показывает превью: "Найдено 250 строк. Валидных: 247. Невалидных: 3."
5. Продавец подтверждает публикацию.

### 14.5. CC Dumps — Полная спецификация

**Типы карт:**

| Тип | Описание | Формат строки |
| :--- | :--- | :--- |
| `with_fullz` | Полные данные | `card;exp_month;exp_year;cvv;zip;name;address;city;state;ssn;dob;phone` |
| `zip_only` | Только ZIP | `card;exp_month;exp_year;cvv;zip` |
| `non_vbv` | Без 3DS | `card;exp_month;exp_year;cvv` |

**BIN-проверка при загрузке:**
1. Система извлекает первые 6 цифр каждой карты.
2. Запрос к BIN-базе (локальная БД или API binlist.net).
3. Определяет: банк-эмитент, страну, тип карты (Visa/MC/Amex), уровень (Classic/Gold/Platinum).
4. Карты с неизвестным BIN помечаются как "Unverified".
...
**Фильтры для покупателя:**
- Банк-эмитент.
- Страна.
- Тип карты (Visa/MC/Amex/Discover).
- Уровень (Classic/Gold/Platinum/Infinite).

### 14.6. eSIM — Полная спецификация
...
| `country` | String | Страна (ISO код + флаг). |
| `data_gb` | Decimal | Объём данных в GB. |
| `duration_days` | Integer | Срок действия в днях. |
| `type` | Enum | `data_only` / `data_calls_sms`. |
| `price` | Decimal | Цена в USD. |
| `is_active` | Boolean | Активен ли тариф. |

**Флоу выдачи eSIM:**
1. Покупатель оплачивает.
2. Заказ появляется в Admin Panel → eSIM Orders.
3. Администратор загружает QR-код или файл активации.
4. Система автоматически отправляет файл покупателю в Mirror Bot.

### 14.7. Accounts (Предзаказ) — Полная спецификация

**Типы аккаунтов:**

| Тип | Подтипы | Описание |
| :--- | :--- | :--- |
| Proxy | Residential / Datacenter / Mobile | Прокси-аккаунты. |
| AI | ChatGPT Plus / Claude Pro / Gemini Advanced / Midjourney | AI-сервисы. |

**Флоу предзаказа:**
1. Покупатель выбирает тип и подтип.
2. Оплачивает.
3. Получает сообщение: "Ваш предзаказ принят. Аккаунт будет доставлен в течение 24-48 часов."
4. Администратор видит заказ в Admin Panel → Pre-orders.
5. Загружает данные аккаунта.
...
### 15.2. Программа лояльности

| Уровень | Условие | Скидка |
| :--- | :--- | :--- |
| Bronze | 0 — $99 потрачено | 0% |
| Silver | $100 — $499 потрачено | 3% |
| Gold | $500 — $1,999 потрачено | 5% |
| Platinum | $2,000+ потрачено | 8% |
...
Прибыль = Сумма_заказа × Комиссия
         ...
         - Скидка_лояльности (0-8% от заказа)
...
### 1.2. Система уровней доступа по депозиту

Вводится многоуровневая система доступа для продавцов, основанная на размере их депозита. Это стимулирует продавцов вносить больший депозит и открывает им доступ к расширенным возможностям.

| Уровень | Депозит | Доступные возможности |
| :--- | :--- | :--- |
| **Basic** | $600 | - Продажа товаров во всех категориях.<br>- 1 помощник в команде.<br>- Базовая статистика. |
| **Advanced** | $1,500 | - Всё из Basic.<br>- 3 помощника в команде.<br>- Расширенная аналитика в Mini App.<br>- API-доступ для автоматизации загрузки. |
| **Pro** | $5,000 | - Всё из Advanced.<br>- 5 помощников в команде.<br>- Приоритетная поддержка.<br>- Сниженная комиссия платформы (12% вместо 15%). |

**Логика покупки/апгрейда:**
- При регистрации продавец вносит минимальный депозит $600.
- В Seller Bot/Mini App есть кнопка **[🚀 Upgrade Access]**.
- При нажатии бот показывает таблицу уровней и предлагает пополнить баланс на недостающую сумму (например, $1500 - $600 = $900 для апгрейда до Advanced).
- После пополнения уровень продавца автоматически повышается.

## 2. База Данных

### 2.1. Новые таблицы

- **`seller_levels`**: `(level_id, level_name, deposit_required, commission_rate)` — для хранения настроек уровней доступа.
...
- **`sellers`**: добавить поле `level_id` (связь с `seller_levels`).
...
Все спецификации и конфигураторы для 9 категорий товаров остаются без изменений по сравнению с v15. Система уровней доступа продавца **не влияет** на то, какие товары он может продавать, а только на комиссию и дополнительные возможности (аналитика, API).
